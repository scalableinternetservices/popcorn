...
resource "aws_ecs_cluster" "popcorn" {
  name = "popcorn"
}

resource "aws_ecr_repository" "popcorn" {
  name                 = "popcorn"
...
module "webserver" {
  ...
  ecr_repository = aws_ecr_repository.popcorn.repository_url
  ecs_cluster    = aws_ecs_cluster.popcorn.id
